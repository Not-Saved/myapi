package it.loris.chess.chesslogic.util;
...
public class MyMath {
	...
	public static boolean isStraight(int[] distance) {
		...
		return (Arrays.equals(versor, Vx()) || Arrays.equals(versor, Vy()));
	}
	
	public static boolean isDiagonal(int[] distance) {
		int[] absDistance = (arrayAbs(distance));
		return (absDistance[0] == absDistance[1]);
	}
	
	public static int squareDistance(int[] distance) {
		int[] absDistance = (arrayAbs(distance));
		if (!isStraight(distance) && !isDiagonal(distance)) throw new IllegalArgumentException("Can't determine a path!");
